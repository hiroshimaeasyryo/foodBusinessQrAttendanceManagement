...
- **転記用** シート: 別Spreadsheetと同じ列構成（`年月`, `スタッフID`, `氏名`, `日付`, `区分`, `出勤開始`, `出勤終了`, `休憩時間(時間)`, `勤務時間(時間)`, `備考`）。確定フラグは転記先で使うため本シートには設けない。日次転記バッチ `runDailyTransfer(対象日)` で打刻記録と各スタッフシートの「休み」を重複なく追記。対象日省略時は「昨日」。
- 各スタッフ用シート: シート名は氏名の「姓」（先頭の単語）。列 `日付`, `始業`, `就業`, `休憩時間(時間)`, `休み` など（1–2行目はヘッダー。値は時間）。「休み」は手動入力し、転記バッチで転記用シートに区分として反映される。
