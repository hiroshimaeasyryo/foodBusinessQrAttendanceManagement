...
- 各スタッフ用シート: シート名は氏名の「姓」（先頭の単語）。列 `日付`, `始業`, `就業`, `休憩時間(時間)`, `休み`, `備考` など（1–2行目はヘッダー。値は時間）。「休み」は手動入力し、転記バッチで転記用シートに区分として反映される。「備考」がある場合は転記用の備考列にそのまま転記される。
...
`runDailyTransferScheduled` は「昨日」を対象日に転記し、**過去60日分**について転記用シートの既存行を打刻記録・スタッフシートの現在の内容と照合し、変更があれば上書きする（休み列の修正や後から追加した行も反映される）。対象日のみ追記したい場合は手動で `runDailyTransfer('yyyy-MM-dd')` を実行すればよい。過去反映の日数は `runDailyTransfer(対象日, { backwardDays: 90 })` のように第2引数で変更できる。
